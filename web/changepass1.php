				...
				<center><div class="col-lg-5 mail-grid1-form ">
						...
						<form method="POST">
							...
							<input type="email"	 name="mailid" placeholder="Enter Email id" disabled required="" value="<?php
								//session_start();
								$mail=$_SESSION['for_user'];
   								echo $mail;
							?>" />
							<input type="Password"	 name="pass" placeholder="Enter New password" required="" />
							<input type="password"	 name="repass" placeholder="Enter Confirm Password" required="" />
							<input type="submit" name="clk" value="Submit"><br/>
							<a href="contact.html" class="ab">Need Help?</a>
						</form>
					</div>
...
if(isset($_POST['clk']))
{
 ...
 if($a==$b)
 {
		$que="UPDATE user SET password='".$a."'WHERE email='".$mail."'";
		$ss=mysqli_query($con,$que);
		if($ss)
		{
			echo "<script>alert('Passwrod change succuse fully');</script>";
		}
		else
		{
			echo "<script>alert('Passwrod not change');</script>";	
		}
 }
 else
 {
 	echo "<script>alert('sorry both password are not same');</script>";
 }
} 
?>
</body>
